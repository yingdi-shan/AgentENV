...
After image import, the node stops BuildKit. The shared sandbox deletion path
freezes writable volume filesystems, seals and publishes their layers, then
stops the VM, without capturing memory, rootfs, or VM state. This also applies
to ordinary sandboxes with mounted volumes. Filesystems stay frozen until the
VM exits, so the next sandbox can mount them without journal recovery.
A failed shutdown, freeze, or cache capture keeps the previous shared seed.
For ordinary sandboxes, recoverable capture or publication failures resume
filesystem writes and retain the running sandbox and its volume reservations.
