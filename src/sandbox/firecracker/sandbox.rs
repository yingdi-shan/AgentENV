...
pub struct FirecrackerSandbox {
    ...
    frozen_volume_mounts: Vec<PathBuf>,
...
impl SandboxBackend for FirecrackerSandbox {
    ...
    async fn freeze_and_snapshot_volumes(&mut self) -> SandboxCaptureResult<()> {
        ...
        let result = async {
            ...
            for mount in mounts {
                ...
                self.frozen_volume_mounts.push(mount);
            }
    ...
    async fn thaw_volumes(&mut self) -> Result<()> {
        while let Some(mount) = self.frozen_volume_mounts.last().cloned() {
            let output = self.set_volume_freeze(&mount, false).await?;
            anyhow::ensure!(
                output.exit_code == 0,
                "thaw volume {} failed: {}",
                mount.display(),
...
impl FirecrackerSandbox {
    ...
    async fn set_volume_freeze(
        ...
        let mount = mount.to_string_lossy().into_owned();
        // Check the mount before freezing so an unmounted volume cannot
        // accidentally freeze the root filesystem hosting envd itself.
        let script = if frozen {
            "/agentenv/bin/busybox mountpoint -q \"$1\" && /agentenv/bin/busybox fsfreeze --freeze \"$1\""
        } else {
            "/agentenv/bin/busybox fsfreeze --unfreeze \"$1\""
        };
        tokio::time::timeout(std::time::Duration::from_secs(30), async move {
            Executor::new(envd)
                ...
                .run_command_with_opts(
                    ...
                    &["sh", "-c", script, "freeze-volume", &mount],
                    &crate::sandbox::ProcessOpts::default().with_cwd("/"),
                )
                .await
        })
