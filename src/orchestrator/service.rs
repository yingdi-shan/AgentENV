        ...
        let delete_result = async {
            if let Some(handle) = handle.as_ref() {
                if previous_state == SandboxState::Running && !volume_ids.is_empty() {
                    match handle.lock().await.freeze_and_snapshot_volumes().await {
                        Ok(()) => volumes_frozen = true,
                        Err(error) if template_builder => {
            ...
            if volumes_captured {
                if let Err(error) = self.publish_sandbox_volume_backings(sandbox_id, &volume_ids).await {
                    if !template_builder {
                        return Err(SandboxCaptureError::recoverable(error.into()));
                    }
                    volumes_captured = false;
            ...
            if let Some(handle) = handle.as_ref() {
                handle.lock().await.stop().await.map_err(SandboxCaptureError::terminal)?;
            }
            ...
            if let Some(manager) = self.volume_manager.as_ref() {
                manager.replace_owner_for(&sandbox_id.to_string(), None, &volume_ids)
                    .await.map_err(|error| SandboxCaptureError::terminal(error.into()))?;
            }
            Ok(())
        }.await;
